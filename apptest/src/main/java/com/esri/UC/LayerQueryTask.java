...
import com.esri.UC.PopupFragment;
...
public class LayerQueryTask {
    ...
    // Query each layer in the mapview in async tasks then display result in customized pop-ups.
    public void queryLayers(float x, float y, int tolerance, boolean showGeometryInfo) {
        if ((mMapView == null) || (!mMapView.isLoaded()) || (mPopupFragment == null))
            return;

        this.mShowGeometryInfo = showGeometryInfo;

        // Loop through each layer in the mapview
        Envelope env = new Envelope(mMapView.toMapPoint(x, y), tolerance * mMapView.getResolution(), tolerance * mMapView.getResolution());
                ...
                if(sublayers!=null){
                    for(Layer flayer:sublayers){
                        ...
                        if (featureLayer.getPopupInfo() != null) {
                            // Query feature layer which is associated with a pop-up definition.
...
//                    // Check if a sub-layer is visible by checking its visibility and tracing back to its parent.
//                    // A sub-layer's visibility depends on the visibility of itself and its parent. When a sub-layer
//                    // is visible but its parent is invisible, its visibility is false. So need to track back to its parent.
//                    ArcGISLayerInfo info = layerInfo;
//                    // If a sub-layer is visible then get hold of its parent and check the parent's visibility.
//                    // Continue till the current sub-layer is invisible or reaches the root.
...
//                        // Query sub-layer which is associated with a pop-up definition and is visible and in scale range.
//                        String url = layer.getQueryUrl(subLayerId);
//                        if ((url == null) || (url.length() < 1))
//                            url = layer.getUrl() + "/" + subLayerId;
//                        mCount.incrementAndGet();
//                        new RunQueryDynamicLayerTask(env, layer, layerInfo.getId(), layer.getSpatialReference()).execute(url);
//                    }
//                }
//            }
        }
    ...
    private PopupLayoutInfo createPopupLayout(Context context) {
        ...
        ViewGroup xmlLayout = (ViewGroup) ((Activity)context).getLayoutInflater().inflate(R.layout.popup_layout, null);
        ...
        layoutInfo.setTitleViewPlaceHolder((ViewGroup) xmlLayout.findViewById(R.id.title_view_placeholder));
        layoutInfo.setAttributesViewPlaceHolder((ViewGroup) xmlLayout.findViewById(R.id.attribute_view_placeholder));
        //layoutInfo.setMediaViewPlaceHolder((ViewGroup) xmlLayout.findViewById(R.id.media_view_placeholder));
        //layoutInfo.setAttachmentsViewPlaceHolder((ViewGroup) xmlLayout.findViewById(R.id.attachment_view_placeholder));
    ...
    private void createViews(Popup popup) {
        ...
        popupLayout.setTitleView(new ArcGISTitleView(context, popup, mMapView));

//        // Create a view for attributes
        popupLayout.setAttributesView(new ArcGISAttributeView(context, popup));
        ArcGISAttributeView attributeView = (ArcGISAttributeView) popupLayout.getAttributesView();
//        // Create an adapter for the attributes in read-only mode USING DEFAULT
        attributeView.setReadOnlyAdapter(new ArcGISReadOnlyAttributesAdapter(context, popup));
//        // Create an adapter for the attributes in edit mode
//        attributeView.setEditAdapter(new MyEditAttributesAdapter(context, popup));
//
//        // Change the view for the attachments
//        if ((popup.getPopupInfo() != null) && (popup.getPopupInfo().isShowAttachments())) {
//            popupLayout.setAttachmentsView(new MyAttachmentsView(context, popup));
//        }
//
...
//        popupLayout.setMediaView(new MyMediaView(context, popup));
    ...
    private String getGeometryInfo(Graphic graphic, ArcGISValueFormat valueFormat) {
        Geometry geometry = graphic.getGeometry();
        String geometryInfo = null;
        if (geometry == null) {
            geometryInfo = "Missing location";
        } else {
            String unit = mMapView.getSpatialReference().getUnit().getDisplayName();
            switch (graphic.getGeometry().getType()) {
                case ENVELOPE:
                case POLYGON:
                case MULTIPOINT:
                    geometryInfo = "Area: " + valueFormat.formatValue(new Double(geometry.calculateArea2D()))
                            + " Square " + unit + "s";
                    break;
                case LINE:
                case POLYLINE:
                    geometryInfo = "Distance: " + valueFormat.formatValue(new Double(geometry.calculateLength2D()))
                            + " " + unit + "s";
                    break;
                case POINT:
                default:
                    geometryInfo = "X: " + valueFormat.formatValue(new Double(((Point) geometry).getX()))
                            + ", Y:" + valueFormat.formatValue(new Double(((Point) geometry).getY()));
            }
        }
    ...
    private class RunQueryFeatureLayerTask extends AsyncTask<ArcGISFeatureLayer, Void, Graphic[]> {
        ...
        protected void onPostExecute(Graphic[] graphics) {
            ...
            for (Graphic gr : graphics) {
                PopupLayoutInfo layout = createPopupLayout(mMapView.getContext());
                Popup popup = featureLayer.createPopup(mMapView, 0, gr, layout, false);
                createViews(popup);

                if (mShowGeometryInfo) {
                    ArcGISValueFormat valueFormat = new ArcGISValueFormat(popup);
                    ArcGISTitleView titleView = (ArcGISTitleView) popup.getLayout().getTitleView();
